...
const TreatmentSlideList = ({ data, loading, onCardClick }: Props) => {
  ...
  const mapToRows = (list: any[]): CardRow[] =>
    (list || []).map((t: any) => {
      const department = subjectMapping[t.subject as keyof typeof subjectMapping] ?? '진료';
      ...
      const speciesKo =
        ...
        '반려동물';
      const agePart = t.pet?.age != null ? `${t.pet.age}세` : t.petInfo?.age != null ? `${t.petInfo.age}세` : '';
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // 숫자 슬롯 가진 항목 상세로 보강
        const needFix = raw.filter((it: any) => typeof it.startTime === 'number' || typeof it.endTime === 'number');
        let merged: any[] = raw;
        if (needFix.length > 0) {
          ...
          merged = raw.map((it: any) => {
            ...
            return {
              ...
              aiSummary:
                it.aiSummary ?? it.ai_summary ?? d?.aiSummary ?? d?.ai_summary ?? it.summaryText ?? d?.summaryText,
            };
          });
        }
