...
const treatments: { [petName: string]: Treatment[] } = {
  미료: [
    {
      reservationId: 1,
      owner: {
...
export default function OwnerReservationDetail() {
  ...
  const treatment = treatments[petName].find((item) => item.reservationId === Number(id));
  ...
  if (!treatment) {
    return (
      <>
        <BackHeader text="진료 정보" />
        <div className="pt-8 h4 text-center text-gray-500">진료 정보를 불러올 수 없습니다.</div>
      </>
    );
  }
      ...
      <section className="flex flex-col gap-6 mt-4 px-7">
        ...
        <div className="flex justify-between border-b-1 border-gray-100 pb-4 ">
          <div className="flex flex-col gap-2">
            <div className="h4">{treatment.pet.name}</div>
            <div className="flex gap-1">
              <div className="p text-center">{treatment.pet.species}</div> /
              <div className="p">{treatment.pet.age}세</div> /<div className="p">{treatment.subject}</div>
            </div>
          </div>
        </div>
        <div className="flex flex-col border-b-1 border-gray-100 pb-4 gap-2">
          <div className="h4">수의사</div>
          <div className="p">{treatment.vetName} 수의사</div>
        </div>
        <div className="flex flex-col border-b-1 border-gray-100 pb-4 gap-2">
          <div className="h4">병원</div>
          <div className="p">{treatment.hospitalName}</div>
        </div>
        <div className="flex flex-col border-b-1 border-gray-100 pb-4 gap-2">
          <div className="h4">예약 희망 시간</div>
          <div className="p">{treatment.reservationTime}</div>
        </div>
        <div className="flex flex-col border-b-1 border-gray-100 pb-4 gap-2 ">
          <div className="h4">증상</div>
          <div>
            <ImageInputBox src={treatment.photo} />
          </div>
          <div className="p">{treatment.description}</div>
        </div>
      </section>
    </div>
  );
